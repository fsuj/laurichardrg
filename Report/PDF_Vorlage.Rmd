...
To compare the rock glacier's annual displacement rates we used one DEM, acquired by an ALS survey on 16./17. August 2012 with a vertical accuracy of 2cm and co-registered to a TLS-based DEM from 2005 [@bodin_multi-annual_2018]. The second DEM is derived from a UAV survey in 2017 and processed by Structure-from-Motion techniques (Jason Goetz, personal communication). Both models have been georeferenced and resampled to a 10cm resolution, but have not been co-registered (JG, personal communication). A first inspection with simple DEM differencing (DoD) showed good accordance between the two data sets in supposedly stable areas around the Laurichard rock glacier. Therefore, we decided to omit a co-registration procedure.

From the DEMs we derived common derivats: Slope, Aspect and Hillshade with GIS standard settings as well as a principle component analysis in order to compare algorithm performance based on different input data.

As validation data we used 
...
* total station and DGPS measurements of marked blocks (p.6) since 1992, we used TODO: number of used points?
* TODO: n own mapped points over the whole area
