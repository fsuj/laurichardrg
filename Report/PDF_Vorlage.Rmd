...
```{r area, echo=FALSE, fig.align="center", out.width = "300px", fig.cap= "Overview of the study site: The Laurichard rock glacier. Input data for algorithms and validation data sets are indicated as points."}
...
For BUnwarpJ, we manually set 61 landmarks in the compared images in order to identify features of local deformations on the rock glacier surface as well as in stable areas. Due to software limitations, we resampled input data to a radiomentric resolution of 8bit. In order to analyse the sensitivity of the BUnwarpJ optimization algorithm, we varied
...
3. the weights of global and local (landmarks) image characteristics, ranging in a scale from 4:0 to 0:4 (landmark weight : image weight).
...
```{r landmarks,echo=FALSE, fig.pos="t!",fig.align="center", out.width = "420px", fig.cap= "Influence of extreme Landmarks on BUnwarpJ performance with DEM as input data. Left: 3D annual displacement values excluding one higly influential boulder. Right: 3D annual displacement values with high displacement at the rock glacier front lobe."}
...
BUnwarpJ results in an overall mean annual displacement of 0.68m (standard deviation of 0.37m/a) and a median annual displacement of 0.64m (interquartile range IQR of 0.58m/a). IMCORR results in higher mean annual displacement rates of 0.79m (sd of 0.51m/a) and a higher median of 0.87m/a (IQR of 0.9m/a). We find the results to differ accordingly in maximum annual displacements of 1.72m and 2.38m, respectively (table \ref{tab:compare_stats}).

IMCORR results with an R² of 0.78 (RMSE 0.3479) seem to be more in accordance with the reference data of @bodin_multi-annual_2018, compared to BUnwarpJ results with R² of 0.7115 (RMSE 0.57). A validation with manually mapped displacements results in an R² of 0.865 (RSME 0.2053) for IMCORR and an R² of 0.7063 (RMSE 0.2414) for BUnwarpJ. BUnwarpJ slightly underestimates the reference values of both validation data sets, while single displacement values of IMCORR overestimate the reference data (figure \ref{fig:compare_scatter})
...
```{r compare, echo=FALSE, fig.pos="t!",fig.align="center", out.width = "420px", fig.cap= "Comparison of annual displacement velocity results. Left: BUnwarpJ with DEM as input data, 60 user-set landmarks and a landmark-global weighting of 1:1. Right: Interpolated IMCORR results with 25.6m search chip size and 6.4m reference chip size. Isolated areas of high displacement in BUnwarpJ (left) and high displacement velocities at the front lobe and in rockfall areas from IMCORR (right) are recognizable."}
knitr::include_graphics("images/Map_3.png")
```


```{r compare_scatter, echo=FALSE, fig.pos="h",fig.align="center", out.width = "370px", fig.cap= "Scatterplots of algorithm results at validation data points vs. validation data. Upper: BUnwarpJ; Lower: IMCORR algorithm. Left: GPS-measured boulders; Right: Manually mapped validation data, respectively."}
...
text_tbl = read.csv(file = "tables/final_raster_stats.csv",header = T,sep = ";")
 knitr::kable(text_tbl, format = "latex", booktabs = TRUE, caption = "Statistics of best performing parameter settings. Left: BUnwarpJ. Right: IMCORR.") %>% 
...
Both algorithm results exhibit similar areas of maximum displacement at the rock glacier tongue when compared to previous work. However, BUnwarpJ results in relatively isolated areas of displacement, which we consider unrealistic. IMCORR, in comparison, results in smoother deformation velocities on the rock glacier tounge. However, the algorithm detects high displacement rates in areas of rockfall while BUnwarpJ automatically and correctly ignores those areas. The high displacement velocities at the steep front lobe of IMCORR seem to be more in accordance with real movements of falling boulders; BUnwarpJ detects hardly any displacement at the front lobe, instead. With respect to the entire study area, we consider IMCORR results with a median displacement of 0.87m/a to be more in accordance with the validation data at hand as well with result maps in @bodin_multi-annual_2018, while BUnwarpJ slightly underestimates displacement velocities (median of 0.64m/a).

Relatively high RMS errors for both algorithms could be due to the spatial resolution of 0.1m of the resulting displacement velocity maps, although IMCORR results have been interpolated beforehand. In addition, we compared areal interpolated data to point data with sample sizes of 18 and 31, respectively. Thus, quality-of-fit measures should be interpreted with caution. Nevertheless, differences in R² and RMSE between the algorithms reflect differences from a visual comparison.

While choosing DEMs, Hillshades or principal components of DEM derivates has hardly any influence on BUnwarpJ results, the algorithm seems to be highly dependent on user input on weighting of local displacement landmarks and global image properties. Putting heavier weights on global charactersitics lowers quality-of-fit measures within the scope of this study. We assume that the necessary conversion of input images to 8bit radiometric resolution, a software constraint, lessens the ability of the algorithm to compare images without user input from landmarks.

IMCORR is highly dependent on the choice of search and reference window sizes. With maximum displacements of around 2m/a from previous studies [@bodin_multi-annual_2018], setting a minimum search window size for the period between 2012 and 2017 of 10m is straightforward. Considering border effects, we can explain the weak performance of IMCORR with a 12.8m search window size compared to 25.6m.

Thus, we can conclude that both algorithms are highly sensitive on prior assumptions and user input. We highlight especially the dependence of BUnwarpJ on the choice of landmarks as local deformation characteristics.

Computational performance of BUnwarpJ is orders of magnitude faster than IMCORR, while the latter is straightforward to apply. BUnwarpJ requires user input before the application of the algorithm as well as post-processing in order to obtain a deformation velocity field. IMCORR results in a matrix of deformation vectors, requiring interpolation to areal data.
...
We consider IMCORR and, to a lesser extent, BUnwarpJ as suitable for detecting rock glacier displacement velocities for the study area at hand. While interpolation of IMCORR displacements with inverse distance weighting does not result in a smooth displacement field and exhibits erroneous areas, the overall displacement pattern is more in accordance with other studies compared with BUnwarpJ results.

Computational expenses limited the application of IMCORR for comparisons of different input data sets. BUnwarpJ, in contrast, is fast to apply but requires more user input. Both algorithms are higly sensitive to parameter settings and prior assumptions.

For future research in the comparison and performance assessment of image matching algorithms, we would suggest to
1. test the influence of different input data sets on IMCORR results; and
2. compare the results obtained to the performance of other established and new algorithms.
...
text_tbl = read.csv(file = "tables/T1_DEM_60_61_xyz_Bodin_manuell.csv",header = T,sep = ";")
 knitr::kable(text_tbl, format = "latex", booktabs = TRUE, caption = "Result Statistics Test I: Influence of user-defined landmarks on DEM-based BUnwrapJ results for 60 landmarks excluding a high influence landmark and 61 landmarks including it. Upper: Comparison with GPS-measured boulders; lower: manually mapped validation data with respective errors.") %>%
    kableExtra::kable_styling(full_width = F, font_size = 10 , latex_options = c("striped","hold_position","scale_down"))%>%
   column_spec(1, bold = TRUE, width = "2cm") %>%  #color = "blue"
   column_spec(2, width = "2.5cm") %>%
...
text_tbl = read.csv(file = "tables/T2_diff_input_60_XYZ_bodin_manuell.csv",header = T,sep = ";")
 knitr::kable(text_tbl, format = "latex", booktabs = TRUE, caption = "Resulting Statistics Test II: Influence of input data on BUnwrapJ results for 60 landmarks: DEM, Hillshade, Aspect, Slope and first principal component of a PCA including DEM, Hillshade, Aspect and Slope. Upper: Comparison with GPS-measured boulders; lower: manually mapped validation data with respective errors.") %>% 
    kableExtra::kable_styling(full_width = F, font_size = 10 , latex_options = c("striped","hold_position","scale_down"))%>%
   column_spec(1, bold = TRUE, width = "2cm") %>%  #color = "blue"
   column_spec(2, width = "2.5cm") %>%
...
text_tbl = read.csv(file = "tables/T3_Settings_XYZ_Bodin_manuell_untereinander.csv",header = T,sep = ";")
 knitr::kable(text_tbl, format = "latex", booktabs = TRUE, caption = "Resulting Statistics Test III: Influence of user-defined weights on DEM-based BUnwrapJ results for 60 landmarks, ranging from 4:0 to 0:4 in landmark weight : image weight. Upper two rows: Comparison with GPS-measured boulders; lower two rows: manually mapped validation data with respective errors.") %>% 
...
text_tbl = read.csv(file = "tables/XYZ_IMCORR_Bodin_manuell_stats.csv",header = T,sep = ";")
 knitr::kable(text_tbl, format = "latex", booktabs = TRUE, caption = "Resulting Statistics Test IV: Influence of search and reference chip sizes on the accuracy of DEM-based IMCORR results with search chip size, reference chip size and 0.1m grid resolution. Upper: Comparison with GPS-measured boulders; lower: manually mapped validation data with respective errors.") %>% 
    kableExtra::kable_styling(full_width = F, font_size = 10 , latex_options = c("striped","hold_position","scale_down"))%>%
   column_spec(1, bold = TRUE, width = "2cm") %>%  #color = "blue"
   column_spec(2, width = "2.5cm") %>%
   column_spec(3:18, width = "3cm") %>%
